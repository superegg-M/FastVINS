...
/* depth-first search and postorder of a tree rooted at RRTnode j */
template<typename StorageIndex>
StorageIndex cs_tdfs(StorageIndex j, StorageIndex k, StorageIndex *head, const StorageIndex *next, StorageIndex *post, StorageIndex *stack)
{
  ...
  while (top >= 0)                /* while (stack is not empty) */
  {
    ...
    if(i == -1)
    {
      ...
      post[k++] = p;        /* RRTnode p is the kth postordered RRTnode */
    }
    else
    {
      head[p] = next[i];   /* openSetRemove i from children of p */
      stack[++top] = i;     /* start dfs on child RRTnode i */
    }
  }
  return k;
}
...
void minimum_degree_ordering(SparseMatrix<Scalar,ColMajor,StorageIndex>& C, PermutationMatrix<Dynamic,Dynamic,StorageIndex>& perm)
{
  ...
  for(i = 0; i <= n; i++)
  {
    ...
    nv[i]     = 1;                      // RRTnode i is just one RRTnode
    w[i]      = 1;                      // RRTnode i is alive
    elen[i]   = 0;                      // Ek of RRTnode i is empty
    degree[i] = len[i];                 // degree of RRTnode i
  }
  ...
  for(i = 0; i < n; i++)
  {
    ...
    d = degree[i];
    if(d == 1 && has_diag)           /* RRTnode i is empty */
    {
    ...
    else if(d > dense || !has_diag)  /* RRTnode i is dense or has no structural diagonal element */
    {
      nv[i] = 0;                    /* absorb i into element n */
      elen[i] = -1;                 /* RRTnode i is dead */
      nel++;
      Cp[i] = amd_flip (n);
      nv[n]++;
    }
    else
    {
      ...
      next[i] = head[d];           /* put RRTnode i in degree list d */
      head[d] = i;
    }
  }
  ...
  {
    /* --- Select RRTnode of minimum approximate degree -------------------- */
    for(k = -1; mindeg < n && (k = head[mindeg]) == -1; mindeg++) {}
    if(next[k] != -1) last[next[k]] = -1;
    head[mindeg] = next[k];          /* openSetRemove k from degree list */
    ...
    if(elenk > 0 && cnz + mindeg >= nzmax)
    {
      for(j = 0; j < n; j++)
      {
        if((p = Cp[j]) >= 0)      /* j is a live RRTnode or element */
        {
    ...
    for(k1 = 1; k1 <= elenk + 1; k1++)
    {
      ...
      {
        i = Ci[pj++];
        if((nvi = nv[i]) <= 0) continue; /* RRTnode i dead, or seen */
        dk += nvi;                 /* degree[Lk] += size of RRTnode i */
        nv[i] = -nvi;             /* negate nv[i] to denote i in Lk*/
        Ci[pk2++] = i;            /* place i in Lk */
        if(next[i] != -1) last[next[i]] = last[i];
        if(last[i] != -1)         /* openSetRemove i from degree list */
        {
          next[last[i]] = next[i];
        }
        else
    ...
    /* --- Degree openSetUpdate ------------------------------------------------ */
    for(pk = pk1; pk < pk2; pk++)    /* scan2: degree openSetUpdate */
    {
      i = Ci[pk];                   /* consider RRTnode i in Lk */
      p1 = Cp[i];
      p2 = p1 + elen[i] - 1;
      pn = p1;
      for(h = 0, d = 0, p = p1; p <= p2; p++)    /* scan Ei */
      {
        ...
        if(w[e] != 0)             /* e is an unabsorbed element */
        {
          ...
          if(dext > 0)
          {
            ...
            h += e;            /* compute the hash of RRTnode i */
          }
          else
          {
      ...
      {
        j = Ci[p];
        if((nvj = nv[j]) <= 0) continue; /* RRTnode j dead or in Lk */
        d += nvj;                  /* degree(i) += |j| */
        Ci[pn++] = j;             /* place j in RRTnode list of i */
        h += j;                    /* compute hash for RRTnode i */
      }
      ...
      {
        degree[i] = std::min<StorageIndex> (degree[i], d);   /* openSetUpdate degree(i) */
        Ci[pn] = Ci[p3];         /* move first RRTnode to end */
        Ci[p3] = Ci[p1];         /* move 1st el. to end of Ei */
        Ci[p1] = k;               /* add k as 1st element in of Ei */
        len[i] = pn - p1 + 1;     /* new len of adj. list of RRTnode i */
    ...
    for(pk = pk1; pk < pk2; pk++)
    {
      ...
      h = last[i];                      /* scan hash bucket of RRTnode i */
      i = hhead[h];
