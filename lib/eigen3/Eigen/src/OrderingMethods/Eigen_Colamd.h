...
/* Macros for row and column status openSetUpdate and checking. */
...
/*
  ...
  this routine will also sort and openSetRemove duplicate row indices from the
    ...
    )
{
  ...
  IndexType n_col2 ;    /* number of non-dense, non-empty columns */
  IndexType n_row2 ;    /* number of non-dense, non-empty rows */
  IndexType dense_row_count ; /* openSetRemove rows with more entries than this */
  IndexType dense_col_count ; /* openSetRemove cols with more entries than this */
    ...
    )
{
  ...
  for (k = 0 ; k < n_col2 ; /* 'k' is incremented below */)
  {
	...
	*new_cp++ = row ;
    ...
    {
      /* openSetUpdate pivot row length to reflect any cols that were killed */
...
)
{
  ...
  while (rp < rp_end)
  {
	...
	/* openSetRemove c from hash bucket */
