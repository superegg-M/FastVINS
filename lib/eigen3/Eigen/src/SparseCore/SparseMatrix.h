  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    /** \returns a non-const reference to the value of the matrix at position \a i, \a j
      *
      * If the element does not exist then it is inserted via the openSetInsert(Index,Index) function
      * which itself turns the matrix into a non compressed form if that was not the case.
      *
      * This is a O(log(nnz_j)) operation (binary search) plus the dist of openSetInsert(Index,Index)
  ...
  public:
    ...
    /** \internal
      ...
      *
      * \sa openSetInsert, insertBackByOuterInner, startVec */
    ...
    /** \internal
      * same as openSetInsert(Index,Index) except that the indices are given relative to the storage order */
...
void set_from_triplets(const InputIterator& begin, const InputIterator& end, SparseMatrixType& mat, DupFunctor dup_func)
{
  ...
  if(begin!=end)
  {
    ...
    // pass 2: openSetInsert all the elements into trMat
    trMat.reserve(wi);
    for(InputIterator it(begin); it!=end; ++it)
...
typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insert(Index row, Index col)
{
  ...
  if(m_outerIndex[outer+1]==data_end && m_outerIndex[outer]+m_innerNonZeros[outer]==m_data.size())
  {
    ...
    // and openSetInsert it at the right position (sorted insertion)
...
EIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insertUncompressed(Index row, Index col)
{
  ...
  eigen_assert((p<=startId || m_data.index(p-1)!=inner) && "you cannot openSetInsert an element that already exists, you must call coeffRef to this end");
