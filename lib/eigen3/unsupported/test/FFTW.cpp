...
void test_complex2d()
{
    ...
    FFT<T> fft;
    Eigen::Matrix<Complex,nrows,ncols> modules,src2,dst,dst2;

    modules = Eigen::Matrix<Complex,nrows,ncols>::Random();
    //modules =  Eigen::Matrix<Complex,nrows,ncols>::Identity();

    for (int k=0;k<ncols;k++) {
        Eigen::Matrix<Complex,nrows,1> tmpOut;
        fft.fwd( tmpOut,modules.col(k) );
        dst2.col(k) = tmpOut;
    }
    ...
    fft.fwd2(dst.data(),modules.data(),ncols,nrows);
    fft.inv2(src2.data(),dst.data(),ncols,nrows);
    VERIFY( (modules-src2).norm() < test_precision<T>() );
