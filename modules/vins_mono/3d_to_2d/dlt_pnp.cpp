...
namespace vins {
    ...
    void Estimator::dltpnp(ImuNode *imu_i, unsigned int batch_size, unsigned int num_batches) {
        ...
        for (unsigned int n = 0; n < num_batches; ++n) {
            ...
            for (unsigned int i = 0; i < batch_size; ++i) {
                ...
                double u = uv[index].x();
                double v = uv[index].y();
                A.row(2 * i) << p.x(), 0., -u * p.x(),
                        p.y(), 0., -u * p.y(),
                        p.z(), 0., -u * p.z(),
                        1. , 0., -u;
                A.row(2 * i + 1) << 0., p.x(), -v * p.x(),
                        0., p.y(), -v * p.y(),
                        0., p.z(), -v * p.z(),
                        0., 1., -v;
            }
            ...
            for (unsigned int j = 0; j < 5; ++j) {
                ...
                for (unsigned int i = 0; i < batch_size; ++i) {
                    ...
                    double inv_depth = 1. / p_c.z();
                    double u_hat = p_c.x() * inv_depth;
                    double v_hat = p_c.y() * inv_depth;
                    Vec2 e {u_hat - u, v_hat - v};

                    Mat23 N;
                    N << inv_depth, 0., -u_hat * inv_depth,
                            0., inv_depth, -v_hat * inv_depth;

                    Eigen::Matrix<double, 2, 6> J;
                    J.leftCols<3>() = -N * R[n].transpose();
                    J.rightCols<3>() = N * Sophus::SO3d::hat(p_c);
        ...
        // 打印信息
        std::cout << "dltpnp: takse " << pnp_t.toc() << " ms" << std::endl;
        std::cout << "dltpnp: inlier points = " << num_inlier[best_iter] << std::endl;
        std::cout << "dltpnp: score = " << score[best_iter] << std::endl;
        std::cout << "dltpnp: q = " << q[best_iter].w() << ", " << q[best_iter].x() << ", " << q[best_iter].y() << ", " << q[best_iter].z() << std::endl;
        std::cout << "dltpnp: t = " << t[best_iter].transpose() << std::endl;
    }
}
